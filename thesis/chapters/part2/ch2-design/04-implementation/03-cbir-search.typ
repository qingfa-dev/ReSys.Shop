...
1. *Client Validation (Vue 3):* The storefront verifies file format (JPEG, PNG, WebP) and size restrictions ($<= 10$ MB), providing immediate UI validation before dispatching a multipart form request to `POST /api/catalog/storefront/search-by-image`.
