...
*Goal:* Capture, refund, void, and review payments. *Trigger:* the administrator opens a payment detail view for an order. *Related requirements:* PAY-GRP-02, PAY-GRP-03. The flow supports partial or full capture and refund through the gateway using idempotency keys, voiding of authorised-but-uncaptured payments, and filtered payment review; alternatives handle state mismatches and already-captured voids, and exceptions cover gateway rejection or unavailability with safe retry.
...
*Goal:* Create, update, activate, and deactivate payment methods. *Trigger:* the administrator opens payment method configuration. *Related requirements:* PAY-GRP-04. The flow creates, edits, activates, deactivates, or removes methods with name and gateway validation; alternatives warn when a method is in use or has pending payments, and an exception handles concurrent modification.
