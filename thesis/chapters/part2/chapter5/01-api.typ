...
#figure(
  ...
  caption: [Carter Minimal API vs. MVC Controllers comparison],
)
...
+ Each module declares `*FeatureMetadata` types containing route, summary, tags, and *permission descriptor*
+ Endpoints call `.HasPermission(...)` which registers a policy name dynamically
+ `PermissionPolicyProvider` maps policy name → `PermissionRequirement`
+ `PermissionContext` enumerates all allowed permission values as a registry
+ ASP.NET Identity claims store the user's permissions; the handler checks claims against the requirement
...
=== Sample Endpoint: Create Product (Admin)
...
=== Sample Endpoint: Search by Image (Storefront)
...
=== Sample Endpoint: Checkout (Storefront)
...
== HTTP Test Artifacts as Living Documentation
