...
4. *The pluggable model architecture is genuinely useful in practice.* Switching models via one environment variable transforms evaluation into systematic comparison, enabling production A/B testing and graceful fallback.
...
6. *Open-source tools are enough.* Pre-trained open-source models and pgvector provide production-ready visual search without proprietary APIs or specialised hardware.

*Answer to RQ3.* The sidecar architecture successfully isolates ML inference from web application logic. The `EMBEDDING_MODEL` environment variable enables model switching without backend code changes. On CPU, end-to-end search latency stays under one second across all six evaluated models. Independent scaling and fault isolation were achieved without distributed infrastructure overhead, which shows that a polyglot architecture with a dedicated AI sidecar is viable for real-time interactive search on commodity hardware. This viability assumes a team with at least one engineer comfortable operating a Python service alongside the .NET deployment; for a very small team without that capability, the overhead of maintaining a second language runtime may outweigh the architectural benefit.
