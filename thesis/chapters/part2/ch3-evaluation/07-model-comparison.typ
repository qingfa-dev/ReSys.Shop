...
@tbl-comparison presents the combined accuracy and efficiency view. Two distinct clusters emerge when both dimensions are examined simultaneously: a transformer-based top tier and a CNN-based lower tier.
...
4. *The pluggable model architecture is a practical enabler.* Switching models via one environment variable transforms evaluation into systematic comparison, enabling production A/B testing and graceful fallback.
5. *Commodity CPU hardware suffices.* Even CLIP models complete inference within 240 ms; combined with pgvector IVFFlat indexes (2.7-6.5 ms), total end-to-end latency stays under one second.
6. *Open-source tools are sufficient.* Pre-trained open-source models and pgvector deliver production-viable visual search without proprietary APIs or specialised hardware.

*Answer to RQ3.* The sidecar architecture successfully isolates ML inference from web application logic. The `EMBEDDING_MODEL` environment variable enables model switching without backend code changes. On CPU, end-to-end search latency stays under one second across all six evaluated models. Independent scaling and fault isolation were achieved without distributed infrastructure overhead, confirming that a polyglot architecture with a dedicated AI sidecar is viable for real-time interactive search on commodity hardware.
