...
#figure(
  table(
    ...
    [6], [Sidecar unavailable], [Stop ML sidecar; upload image and search.], [#emph[HTTP 503]; catalogue and cart endpoints remain functional.], [Pass],
...
#figure(
  table(
    ...
    [8], [Fashion-CLIP], [#emph[POST /embeddings/bytes] with fashion image.], [512-dim vector; model "fashion_clip"; inference_ms present.], [Pass],
    ...
    [13], [Invalid API key], [#emph[POST /embeddings/bytes] without API key.], [#emph[HTTP 401 Unauthorized].], [Pass],
  ),
  kind: table,
