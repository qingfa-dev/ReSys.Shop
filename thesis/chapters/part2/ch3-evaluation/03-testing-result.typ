...
#figure(
  table(
    ...
    [6], [Sidecar unavailable], [Stop ML sidecar; upload image and search.], [#raw("HTTP 503", lang: "http"); catalogue and cart endpoints remain functional.], [Pass],
...
#figure(
  table(
    ...
    [8], [Fashion-CLIP], [#raw("POST /embeddings/bytes", lang: "http") with fashion image.], [512-dim vector; model "fashion_clip"; inference_ms present.], [Pass],
    ...
    [13], [Invalid API key], [#raw("POST /embeddings/bytes", lang: "http") without API key.], [#raw("HTTP 401 Unauthorized", lang: "http").], [Pass],
  ),
  kind: table,
