...
#figure(
  ...
  caption: [Inference latency comparison across six evaluated models. EfficientNet-B0 leads at 42.6 ms, followed by ResNet-50 (96.6 ms), FashionCLIP (113.6 ms), DINOv2 ViT-S/14 (126.3 ms), CLIP ViT-B/32 (140.5 ms), and CLIP ViT-B/16 (235.5 ms).],
) <fig-benchmark-latency>

#figure(
  image("../../../figures/chapters/part2/ch3-evaluation/diagrams/P2S3.6_benchmark_throughput.png", width: 75%),
  caption: [Throughput comparison across six evaluated models. EfficientNet-B0 leads at 21.4 img/s, followed by FashionCLIP (14.2), CLIP ViT-B/32 (11.9), ResNet-50 (10.2), DINOv2 ViT-S/14 (10.2), and CLIP ViT-B/16 (4.0).],
...
#figure(
  ...
  table(
    ...
    [FashionCLIP], [113.6 ± 3.7], [14.2 ± 0.5], [5 109.9], [*3.3*], [N/A],
    [DINOv2 ViT-S/14], [126.3 ± 5.1], [10.2 ± 0.2], [1 223.4], [*2.4*], [N/A],
    [CLIP ViT-B/32], [140.5 ± 7.8], [11.9 ± 0.2], [1 868.8], [*3.3*], [N/A],
    [CLIP ViT-B/16], [235.5 ± 7.3], [4.0 ± 0.1], [6 518.5], [*3.3*], [N/A],
  ),
  kind: table,
) <tbl-efficiency>

EfficientNet-B0 dominates every efficiency metric. Its inference time of 42.6 ms is 2.2× faster than DINOv2 ViT-S/14 (126.3 ms) and 5.5× faster than CLIP ViT-B/16 (235.5 ms). Its throughput of 21.4 img/s is 1.5× higher than FashionCLIP (14.2) and 2.1× higher than the two CLIP variants' lower range. Load time of 118.3 ms is less than a third of ResNet-50 (385.6 ms) and two orders of magnitude below the multi-second CLIP load times. This profile makes EfficientNet-B0 the only model clearly viable for CPU-only deployment at interactive latencies without cold-start penalties.
...
*Answer to RQ2.* The accuracy-speed trade-off is substantial and non-linear. EfficientNet-B0 (42.6 ms) achieves 95.55% of Fashion-CLIP's mAP (0.9077 vs 0.9336) at 37.5% of the latency. ResNet-50 combines a middling mAP (0.9132) with middling latency (96.6 ms) and the largest storage footprint (13.0 MB). The relationship is not simply "slower equals more accurate": CLIP ViT-B/16 has the highest latency (235.5 ms) yet ranks below the faster FashionCLIP, DINOv2, and CLIP ViT-B/32, demonstrating that architecture and patch configuration, not raw inference time, govern accuracy. Practitioners choosing between EfficientNet-B0 and Fashion-CLIP weigh a 2.86% mAP improvement against a 5.5× latency increase.
