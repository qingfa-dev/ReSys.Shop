...
#figure(
  ...
  table(
    ...
    [Fashion-CLIP], [113.6 ± 3.7], [14.2 ± 0.5], [5,109.9], [*3.3*], [~600],
...
#footnote[\* Estimated from model parameter count plus PyTorch runtime overhead; direct measurement unreliable on the test kernel.]

@tbl-efficiency presents the full efficiency results. EfficientNet-B0 leads every efficiency metric. Its inference time of 42.6 ms is 3.0× faster than DINOv2 ViT-S/14 (126.3 ms) and 5.5× faster than CLIP ViT-B/16 (235.5 ms). Its throughput of 21.4 img/s is 1.5× higher than Fashion-CLIP (14.2) and 2.1× higher than DINOv2 and ResNet-50 (10.2). Load time of 118.3 ms is less than a third of ResNet-50 (385.6 ms) and 16--55x lower than the multi-second CLIP load times. The EfficientNet-B0 latency standard deviation (5.6 ms, CV 13%) is comparable to the other models; any higher variance observed in earlier runs reflected CPU frequency scaling on the single-test hardware and was resolved by pinning the CPU governor before the final benchmark. This profile makes EfficientNet-B0 the strongest candidate for CPU-only deployment at interactive latencies without cold-start penalties.

The CLIP-family and DINOv2 models exhibit different inference latencies driven by self-attention scaling with image patches. Fashion-CLIP (113.6 ms) and CLIP ViT-B/32 (140.5 ms) are markedly faster than CLIP ViT-B/16 (235.5 ms), a consequence of the larger ViT-B/16 patch configuration on this CPU; CLIP ViT-B/16 also has the lowest throughput (4.0 img/s). DINOv2 ViT-S/14 (126.3 ms) sits between the CLIP models. Load times of 1.2-6.5 seconds for the transformer models reflect weight initialisation cost, a one-time startup penalty, not per-request.
...
*Answer to RQ2.* The accuracy-speed trade-off is large and non-linear. EfficientNet-B0 (42.6 ms) achieves 97.2% of Fashion-CLIP's mAP (0.9077 vs 0.9336) at 37.5% of the latency. ResNet-50 combines a middle-range mAP (0.9132) with middle-range latency (96.6 ms) and the largest storage footprint (13.0 MB). The relationship is not simply "slower equals more accurate": CLIP ViT-B/16 has the highest latency (235.5 ms) yet ranks below the faster Fashion-CLIP, DINOv2, and CLIP ViT-B/32, which shows that architecture and patch configuration, not raw inference time, govern accuracy. Practitioners choosing between EfficientNet-B0 and Fashion-CLIP weigh a 2.86% mAP improvement against a 2.67× latency increase.
