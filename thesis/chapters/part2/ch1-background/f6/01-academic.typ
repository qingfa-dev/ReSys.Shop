...
The *Fashion-CLIP* work demonstrated that domain-specific fine-tuning of CLIP on 700,000 fashion images improves retrieval over the general model @chia2022fashionclip. This thesis follows that approach, using pre-trained models without custom training, and extends the evaluation to additional architectures (ResNet, EfficientNet, DINOv2) for systematic comparison. The benchmark in Chapter 3 (§3.5) measured a 1.46% relative mAP advantage of Fashion-CLIP over generic CLIP ViT-B/16 under category-only evaluation.

More recently, unified multimodal embedding models such as ImageBind @girdhar2023imagebind and BLIP-2 @li2023blip2 have pushed beyond the image-text pairing of CLIP, binding several modalities into shared latent spaces. These advances underpin the cross-modal text-plus-image retrieval direction discussed in this thesis's future work (Chapter 4), which would reuse the CLIP-family shared latent space for hybrid queries; they were outside the scope of the single-query benchmark presented here.
