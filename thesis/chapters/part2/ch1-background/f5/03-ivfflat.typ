...
IVFFlat is a simpler ANN algorithm used during model benchmarking. It partitions the embedding space into clusters via *k-means* and stores each vector in its nearest cluster @pgvector2023.
...
IVFFlat is used for the model comparison benchmarks in Chapter 3, where the objective is ranking embedding models rather than optimising index performance. Its fast build time and simple configuration suit rapid benchmarking. For production, HNSW is the designated long-term index.
