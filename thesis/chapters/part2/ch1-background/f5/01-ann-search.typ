...
The solution is *Approximate Nearest Neighbour* (ANN) search @malkov2018efficient. Instead of checking every vector, ANN algorithms build index structures (graphs or clusters) that let a query navigate directly to the neighbourhood of likely matches, skipping irrelevant vectors.
