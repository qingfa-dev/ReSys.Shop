...
The solution is *Approximate Nearest Neighbour* (ANN) search @malkov2018efficient. Instead of checking every vector, ANN algorithms build index structures (graphs or clusters) that let a query go directly to the group of likely matches, skipping irrelevant vectors.

The key trade-off: we trade perfect accuracy for speed. If the true top match has similarity 0.95 and the returned result has 0.93, that is acceptable for product search. ANN algorithms typically achieve 97 to 99% recall of the true top matches while reducing search time by orders of magnitude @malkov2018efficient. Two algorithms are used in this project: *HNSW* for production search and *IVFFlat* for rapid benchmarking @pgvector2023.
