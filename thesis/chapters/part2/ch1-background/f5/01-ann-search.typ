...
The key trade-off: we trade perfect accuracy for speed. If the true top match has similarity 0.95 and the returned result has 0.93, that is acceptable for product search. ANN algorithms achieve *97 to 99% recall* of the true top matches while reducing search time by orders of magnitude. Two algorithms are used in this project: *HNSW* for production search and *IVFFlat* for rapid evaluation.
