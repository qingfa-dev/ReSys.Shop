...
#figure(
  ...
  caption: [Defense-in-depth security controls by layer],
)
...
+ Moving dev secrets to `dotnet user-secrets` (id `resys.shop.api`)
+ `JwtSettingsValidator` rejects known-dev literal secrets in non-Development environments
+ `setup-dev-secrets.sh` bootstraps dev secrets safely
...
+ *Permission descriptors* are defined in `PermissionContext` as:
  - `Domain` (e.g., `catalog`, `ordering`)
  - `Category` (e.g., `products`, `orders`)
  - `Action` (e.g., `create`, `read`, `update`, `delete`, `cancel`)

+ *Feature metadata* types (e.g., `CatalogFeature.Admin.Products.Create`) bundle route, tags, summary, and permission descriptor.

+ *Endpoint registration* calls `.HasPermission(CatalogPermission.AdminProductsCreate)` which creates a policy string like `catalog:products:create`.

+ *Policy provider* (`PermissionPolicyProvider`) resolves the string to a `PermissionRequirement`.

+ *Handler* checks if the user's claims contain the required permission.
...
=== Admin vs Storefront Surface Segregation
...
#figure(
  ...
  caption: [File upload security controls],
)
...
#figure(
  ...
  caption: [Secrets management by environment],
)
