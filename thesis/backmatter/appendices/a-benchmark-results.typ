...
#figure(
  ...
  table(
    ...
    [Fashion-CLIP], [0.9336 ± 0.0060], [0.9607 ± 0.0041], [0.9527 ± 0.0043], [0.9383 ± 0.0036], [0.0282 ± 0.0014], [0.0488 ± 0.0021], [0.0816 ± 0.0031],
...
#figure(
  ...
  table(
    ...
    [Fashion-CLIP], [0.2439 ± 0.0074], [0.4414 ± 0.0106], [0.4041 ± 0.0100], [0.3619 ± 0.0082], [0.0430 ± 0.0017], [0.0717 ± 0.0031], [0.1130 ± 0.0027],
...
#figure(
  ...
  table(
    ...
    [Fashion-CLIP], [0.2071 ± 0.0074], [0.3762 ± 0.0097], [0.3377 ± 0.0090], [0.2952 ± 0.0065], [0.0628 ± 0.0024], [0.1032 ± 0.0044], [0.1586 ± 0.0042],
...
#figure(
  ...
  table(
    ...
    [Fashion-CLIP], [113.6 ± 3.7], [14.2 ± 0.5], [5,109.9], [3.3], [~600], [512],
  ),
  kind: table,
)
...
EfficientNet-B0 achieves the lowest inference latency (42.6 ms) and highest throughput (21.4 images per second), making it the most computationally efficient model. CLIP-based models (Fashion-CLIP, CLIP ViT-B/16, CLIP ViT-B/32) and DINOv2 incur the highest model load times (1.2--6.5 seconds) due to their transformer architectures and larger weight files. ResNet-50 requires the most storage (13.0 MB per 3,334 gallery vectors) owing to its high embedding dimensionality of 2,048. In this benchmark, ResNet-50 embeddings were not indexed with pgvector IVFFlat due to their large dimensionality; only the 512-dim and 1,280-dim models were indexed for the pgvector production measurements (Section 3.7). HNSW or dimensionality reduction would be needed for approximate indexing of 2,048-dim vectors at production scale. RAM values are estimates derived from each model's parameter count plus PyTorch runtime overhead, because direct process-level measurement via psutil proved unreliable on the Linux host used for these experiments. These figures are indicative rather than instrumented; actual values vary with batch size and runtime overhead, scaling from approximately 100 MB (EfficientNet-B0) to over 600 MB (CLIP-based).
...
#figure(
  ...
  table(
    ...
    [Fashion-CLIP], [0.9274], [0.9340], [0.9394], [0.9336 ± 0.0060],
