...
5. **Cross-module communication** uses `ISender.Send(new OtherModule.Command(...))` — e.g., `Checkout` sends `ReserveCartStock.Command` to Inventory module
...
| `Module/{Domain}/Domain/` | Aggregate roots, value objects, enums, domain methods returning Result, error factories | Persistence, API concerns, cross-module references | `service/Api/src/Module/Ordering/Domain/Orders/Order.cs` |
...
| **ISender cross-module** | `sender.Send(new OtherModule.Command(...))` in handler code | Enforces module isolation — modules never reference each other directly |
...
- `service/Api/src/Module/Ordering/Features/Storefront/Cart/Checkout/CreateOrderFromCart.cs` — representative handler with cross-module orchestration
