...
- **SendGrid provider typo resolved** — `SendGrids` key (plural + extra `s`), `SendGird` folder (one `r`), and `SendGrids` C# section path were all normalized to `SendGrid`. The email channel should now dispatch via SendGrid when enabled.
