...
| Medium | Dev JWT secret rejected for non-Development environments | `JwtSettingsValidator` | Production deployment would fail if dev secrets pattern used | Document production secret requirements explicitly |
...
| Stale Embedding build artifacts | `service/Embedding/build/lib/` not gitignored | codebase audit | Confusion about deployable artifacts; git bloat | Add to `.gitignore` and clean |
