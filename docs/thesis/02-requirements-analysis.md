...
2. Storefront POSTs to `/api/admin/catalog/storefront/search-by-image`
...
### 2.4.2 Use Case: Model Comparison Evaluation (Research)
...
4. Record Precision@20, Recall@20, embedding generation time per image, total storage
5. Repeat steps 1–4 for Model B (ResNet-50), Model C (EfficientNet-B0), Model D (CLIP-generic)
6. Compute mean ± SD across all queries per model
