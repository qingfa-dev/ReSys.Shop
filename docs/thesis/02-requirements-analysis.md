...
| CAT-FR-06 | **Image embeddings are generated via Fashion-CLIP** for visual similarity search; embeddings stored in PostgreSQL pgvector | High | `Module/Catalog/Features/Admin/Products/Variants/Images/Embeddings/`, `Shared/Operational/Persistence/Configurations/Vectors/Vector.Configuration.cs` |
...
### 2.4.1 Use Case: Search by Image (CBIR)

**Actor**: Storefront Customer
**Precondition**: Product catalog contains images with embeddings
**Flow**:
...
4. Sidecar returns 512-d Fashion-CLIP embedding vector
5. Backend queries PostgreSQL pgvector with cosine similarity: `SELECT * FROM variant_images ORDER BY embedding <=> $1 LIMIT 20`
...
### 2.4.2 Use Case: Checkout

**Actor**: Authenticated or Guest Customer
**Precondition**: Cart contains items; stock is available
**Flow**:
1. Customer sets address → `CheckoutState = Address`
2. Customer selects shipping method → `CheckoutState = Delivery`
3. Customer selects payment method → `CheckoutState = Payment`
4. Backend creates payment intent (Stripe or Bogus)
5. Customer confirms → `CheckoutState = Confirm`
6. Backend finalizes: creates order, deducts stock, clears cart → `CheckoutState = Complete`
...
3. Should the requirements be formally numbered (e.g., R-001) and cross-referenced throughout the other thesis chapters?
4. Are there specific NFRs the examiner expects (e.g., GDPR compliance, WCAG accessibility) that should be added?
