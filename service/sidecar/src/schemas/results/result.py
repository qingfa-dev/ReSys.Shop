...
from src.schemas.results.failure import Failure
...
class Result(BaseModel):
    ...
    is_success: bool = Field(alias="isSuccess", description="Whether the operation was successful.")
    status_code: int = Field(alias="statusCode", description="The HTTP status code representing the result.", json_schema_extra={"example": 200})
    message: Optional[str] = Field(default=None, description="An optional summary message.")
    failures: List[Failure] = Field(default=[], description="A list of failures, populated if isSuccess is False.")
    ...
    @classmethod
    def failure(cls, failure: Union[Failure, List[Failure]], message: Optional[str] = None) -> "Result":
...
class ValueResult(Result, Generic[T]):
    ...
    value: Optional[T] = Field(default=None, description="The data payload returned by the operation on success.")

    @classmethod
    def ok_value(cls, value: T, status_code: int = 200, message: Optional[str] = None) -> "ValueResult[T]":
        """Creates a successful result containing the specified value."""
        return cls(isSuccess=True, statusCode=status_code, message=message, failures=[], value=value)

    @classmethod
    def failure_value(cls, failure: Union[Failure, List[Failure]], message: Optional[str] = None) -> "ValueResult[T]":
