...
public sealed class GetCountryByIsoIntegrationTests(ApiFixture fixture) : ApiIntegrationTestBase(fixture)
{
    ...
    {
        HttpResponseMessage response = await Client.GetAsync("/api/locations/countries/by-iso/US");
    ...
    {
        HttpResponseMessage response = await Client.GetAsync("/api/locations/countries/by-iso/XX");
        ApiResponse result = await response.ReadApiResponseAsync();
