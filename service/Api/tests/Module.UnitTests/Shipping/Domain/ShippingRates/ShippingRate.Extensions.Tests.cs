...
public class ShippingRateExtensionsTests
{
    ...
    public void Create_WithValidParams_ShouldReturnRate()
    {
        ...
        var r = ShippingRateExtensions.Create("Standard", 9.99m, shipmentId, methodId);
        ...
        r.Value.ShipmentId.Should().Be(shipmentId);
        r.Value.ShippingMethodId.Should().Be(methodId);
    }
    [Fact]
    public void Create_WithZeroCost_ShouldFail()
    {
        var r = ShippingRateExtensions.Create("Free", 0, Guid.NewGuid(), Guid.NewGuid());
        r.IsFailure.Should().BeTrue();
    }
    [Fact]
    public void Create_WithNegativeCost_ShouldFail()
    {
        var r = ShippingRateExtensions.Create("Neg", -5, Guid.NewGuid(), Guid.NewGuid());
        r.IsFailure.Should().BeTrue();
    }
    [Fact]
    public void Select_WhenNotSelected_ShouldSucceed()
    {
        var r = ShippingRateExtensions.Create("Std", 10, Guid.NewGuid(), Guid.NewGuid()).Value;
