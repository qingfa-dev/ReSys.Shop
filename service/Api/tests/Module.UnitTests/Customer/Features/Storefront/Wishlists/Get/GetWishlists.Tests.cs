...
using Module.Customer.Features.Storefront.Wishlists.Get;
...
public class GetWishlistsTests : IDisposable
{
    ...
    public GetWishlistsTests()
    {
        ...
        _handler = new GetWishlists.PagedQueryHandler(_dbContext);
    }
    ...
    public async Task Handle_ShouldReturnWishlists()
    {
        ...
        var result = await _handler.Handle(new GetWishlists.Query(_userId, new GetWishlists.Parameters()), TestContext.Current.CancellationToken);
