...
public class UpdateTaxonRuleTests : IDisposable
{
    private readonly ApplicationDbContext _dbContext;
    private readonly Mock<IAutoClassificationService> _autoClassificationMock;
    private readonly Mock<ILogger<UpdateTaxonRule.CommandHandler>> _loggerMock;
    private readonly UpdateTaxonRule.CommandHandler _handler;

    public UpdateTaxonRuleTests()
    {
        ...
        _loggerMock = new Mock<ILogger<UpdateTaxonRule.CommandHandler>>();

        _handler = new UpdateTaxonRule.CommandHandler(_dbContext, _autoClassificationMock.Object, _loggerMock.Object);
    }
    ...
    public async Task Handle_ShouldReturnSuccess_WhenTaxonNotAutomatic()
    {
        ...
        var request = new UpdateTaxonRule.Request
        {
            Type = "product_price",
        ...
        result.Value.MatchPolicy.Should().Be("greater_than");
        result.Value.Value.Should().Be("50.00");

        var persisted = await _dbContext.Set<TaxonRule>().FindAsync([rule.Id], TestContext.Current.CancellationToken);
        persisted.Should().NotBeNull();
        persisted!.Type.Should().Be(TaxonRuleType.ProductPrice);
        persisted.MatchPolicy.Should().Be(TaxonRuleMatchPolicy.GreaterThan);
        persisted.Value.Should().Be("50.00");
