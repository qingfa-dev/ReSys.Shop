...
using Module.Catalog.Features.Admin.Products.OptionTypes.Assign;

namespace Module.UnitTests.Catalog.Features.Admin.Products.OptionTypes.Assign;
...
public class AssignProductOptionTypesTests : IDisposable
{
    ...
    public async Task Handle_ShouldAddNewJunctions()
    {
        ...
        var request = new AssignProductOptionTypes.Request { Items = [new() { OptionTypeId = optionTypeId, Position = 1 }] };
        var result = await _handler.Handle(new AssignProductOptionTypes.Command(product.Id, request), TestContext.Current.CancellationToken);
    ...
    public async Task Handle_ShouldUpdatePositionOnExisting()
    {
        ...
        var request = new AssignProductOptionTypes.Request { Items = [new() { OptionTypeId = optionTypeId, Position = 5 }] };
        var result = await _handler.Handle(new AssignProductOptionTypes.Command(product.Id, request), TestContext.Current.CancellationToken);
    ...
    public async Task Handle_ShouldReturnFailure_WhenProductNotFound()
    {
        ...
        var result = await _handler.Handle(new AssignProductOptionTypes.Command(Guid.NewGuid(), request), TestContext.Current.CancellationToken);
