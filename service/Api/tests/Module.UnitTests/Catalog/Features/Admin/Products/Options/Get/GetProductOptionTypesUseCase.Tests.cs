...
using Module.Catalog.Features.Admin.Products.Options.Get;

namespace Module.UnitTests.Catalog.Features.Admin.Products.Options.Get;
...
public class GetProductOptionTypesTests : IDisposable
{
    ...
    private readonly GetProductOptionTypes.PagedQueryHandler _handler;
    ...
    public GetProductOptionTypesTests()
    {
        ...
        _handler = new GetProductOptionTypes.PagedQueryHandler(_dbContext);
    }
    ...
    public async Task Handle_ShouldReturnItemsWithCorrectIsAssigned()
    {
        ...
        var result = await _handler.Handle(new GetProductOptionTypes.Query(product.Id, new GetProductOptionTypes.Parameters()), TestContext.Current.CancellationToken);

        result.IsSuccess.Should().BeTrue();
        result.Items.Should().HaveCount(2);
        result.Items.Should().ContainSingle(x => x.OptionTypeId == ot1.Id && x.IsAssigned && x.Position == 3);
        result.Items.Should().ContainSingle(x => x.OptionTypeId == ot2.Id && !x.IsAssigned && x.Position == 0);
    }

    [Fact(DisplayName = "Handler: Should return failure when product not found")]
    public async Task Handle_ShouldReturnFailure_WhenProductNotFound()
    {
        var result = await _handler.Handle(new GetProductOptionTypes.Query(Guid.NewGuid(), new GetProductOptionTypes.Parameters()), TestContext.Current.CancellationToken);
    ...
    public async Task Handle_ShouldNotBleedFromOtherProducts()
    {
        ...
        var result = await _handler.Handle(new GetProductOptionTypes.Query(productB.Id, new GetProductOptionTypes.Parameters()), TestContext.Current.CancellationToken);

        result.IsSuccess.Should().BeTrue();
        result.Items.Should().ContainSingle(x => x.OptionTypeId == ot.Id && !x.IsAssigned);
    }

    [Fact(DisplayName = "Handler: Should page option types when parameters supplied")]
    public async Task Handle_ShouldPage_WhenParametersSupplied()
    {
        var product = ProductMethod.Create("Test Product", "test-product").Value;
        var ot1 = OptionTypeMethod.Create("Color", "Color", 0).Value;
        var ot2 = OptionTypeMethod.Create("Size", "Size", 0).Value;
        var ot3 = OptionTypeMethod.Create("Material", "Material", 0).Value;

        _dbContext.Set<Product>().Add(product);
        _dbContext.Set<OptionType>().AddRange(ot1, ot2, ot3);
        _dbContext.Set<ProductOptionType>().Add(ProductOptionTypeMethod.Create(product.Id, ot1.Id, position: 1).Value);
        await _dbContext.SaveChangesAsync(TestContext.Current.CancellationToken);

        var result = await _handler.Handle(
            new GetProductOptionTypes.Query(product.Id, new GetProductOptionTypes.Parameters { PageSize = 2 }),
