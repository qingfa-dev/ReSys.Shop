...
using Module.Catalog.Features.Storefront.Taxonomies.Taxons.Get;

namespace Module.UnitTests.Catalog.Features.Storefront.Taxonomies.Taxons.Get;
...
public class ListTaxonsTests : IDisposable
{
    ...
    private readonly GetStoreTaxons.PagedQueryHandler _handler;
    ...
    public ListTaxonsTests()
    {
        ...
        _handler = new GetStoreTaxons.PagedQueryHandler(_dbContext);
    }
    ...
    public async Task Handle_ShouldReturnList()
    {
        ...
        var result = await _handler.Handle(new GetStoreTaxons.Query(new GetStoreTaxons.Parameters()), TestContext.Current.CancellationToken);
    ...
    [Fact(DisplayName = "Handler: Should return all taxons")]
    public async Task Handle_ShouldReturnAllTaxons()
    {
        var taxonomy = new Taxonomy { Name = "Categories" };
