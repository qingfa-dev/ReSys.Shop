...
from embedding.core.config import Settings, LogLevel, Environment
...
def test_otlp_endpoint_url_validation():
    ...
    # Valid http
    assert Settings(OTEL_EXPORTER_OTLP_ENDPOINT="http://collector:4317").OTEL_EXPORTER_OTLP_ENDPOINT == "http://collector:4317"
    # Valid https
    assert Settings(OTEL_EXPORTER_OTLP_ENDPOINT="https://secure:4317").OTEL_EXPORTER_OTLP_ENDPOINT == "https://secure:4317"
...
def test_log_level_validation():
    ...
    assert "LOG_LEVEL must be one of ['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL']" in str(excinfo.value)
