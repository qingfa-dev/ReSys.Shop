...
describe('usePagedList', () => {
  ...
  function createMockFetch(result: Partial<ServerPagedResult<MockItem>> = {}) {
    return vi.fn<() => void>().mockResolvedValue({
      isSuccess: true,
      statusCode: 200,
      errors: [],
      message: null,
  ...
  it('should handle empty data response', async () => {
    const mockFetch = vi.fn<() => void>().mockResolvedValue({ isSuccess: true, statusCode: 200, errors: [], message: null, metadata: null, items: [], page: 1, pageSize: 10, totalCount: 0 })
  ...
  it('loading should be true during fetch', async () => {
    ...
    const mockFetch = vi.fn<() => void>().mockImplementation(() => new Promise(resolve => {
