...
const loadOptionData = async () => {
    loading.value = true;
    try {
        const response = await productRepository.getOptionTypes(props.productId);
...
const confirmGeneration = async () => {
    ...
    try {
        for (const variant of generatedPreview.value) {
            ...
            const createRes = await variantRepository.create(props.productId, {
                sku: payload.sku,
                price: payload.price,
                position: 0,
                trackInventory: true
            ...
            if (createRes.isSuccess && createRes.value) {
                ...
                await variantRepository.syncOptionValues(variantId, payload.optionValues);
                successCount++;
            }
        }
