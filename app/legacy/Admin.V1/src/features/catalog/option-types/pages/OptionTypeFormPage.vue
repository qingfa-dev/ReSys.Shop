...
import { useOptionValueStore } from '@/features/catalog/option-values'
import { storeToRefs } from 'pinia'
import { useForm } from 'vee-validate'
import { toTypedSchema } from '@vee-validate/zod'
import { createOptionValueSchema } from '@/features/catalog/option-values'
...
import { createOptionTypeSchema } from '../types'
...
<template>
  <div class="flex flex-col h-full overflow-hidden">
    <div
      class="flex items-center justify-between mb-4 bg-surface-0 dark:bg-surface-900 p-4 rounded-2xl border border-surface-100 dark:border-surface-800 shadow-sm">
      <div class="flex items-center gap-3 overflow-hidden">
        <div class="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary shrink-0">
          <i :class="isEdit ? 'pi pi-pencil' : 'pi pi-plus'"></i>
        </div>
        <div class="overflow-hidden">
          <h3 class="text-lg font-black tracking-tight m-0 truncate">{{ isEdit ? (presentation || 'Edit Option Type') :
            t('catalog.option_types.titles.create') }}</h3>
          <p class="text-xs text-surface-500 m-0 truncate">{{ isEdit ? 'Updating configuration and values' :
            t('catalog.option_types.descriptions.create') }}</p>
        </div>
      </div>
      <div class="flex items-center gap-2 shrink-0">
        <Button
          :label="isEdit ? t('catalog.option_types.actions.save_edit') : t('catalog.option_types.actions.save_create')"
          icon="pi pi-check" class="rounded-xl px-6 shadow-lg shadow-primary/20" :loading="loading" @click="onSubmit" />
      </div>
    </div>
                ...
                <div class="flex flex-col gap-6">
                  <div class="grid grid-cols-1 gap-6">
                    <FormField :label="t('catalog.option_types.labels.name')" name="name" :error="errors.name">
                      <InputText v-model="name" class="w-full rounded-xl h-11" :invalid="!!errors.name"
                        :placeholder="t('catalog.option_types.placeholders.name')" />
                    </FormField>

                    <FormField :label="t('catalog.option_types.labels.presentation')" name="presentation"
                      :error="errors.presentation">
                      <InputText v-model="presentation" class="w-full rounded-xl h-11" :invalid="!!errors.presentation"
                        :placeholder="t('catalog.option_types.placeholders.presentation')" />
                    </FormField>

                    <div class="grid grid-cols-2 gap-4">
                      <FormField :label="t('catalog.option_types.labels.position')" name="position">
                        <InputNumber v-model="position" showButtons :min="0" class="w-full rounded-xl overflow-hidden"
                          inputClass="h-11" />
                      </FormField>
                      <FormField :label="t('catalog.option_types.labels.filterable')" name="filterable">
                        <div class="flex items-center gap-2 h-11">
                          ...
                          <span class="text-xs text-surface-500">{{ filterable ? 'Visible in filters' : 'Internal only'
                            }}</span>
                        </div>
                      </FormField>
                    </div>
              ...
              <TabPanel v-if="isEdit" :value="1">
                <div class="flex flex-col gap-4">
                  <div class="flex items-center justify-between">
                    ...
                    <Button :label="t('catalog.option_types.actions.add_value')" icon="pi pi-plus" size="small" text
                      @click="openNewValue" />
                  </div>
                  <DataTable :value="optionValues" :loading="valueLoading" size="small" scrollable rowHover
                    class="rounded-2xl border border-surface-100 dark:border-surface-800 overflow-hidden">
                    <Column field="name" :header="t('catalog.option_types.labels.value_name')"
                      class="font-bold text-xs"></Column>
                    <Column field="presentation" :header="t('catalog.option_types.labels.value_presentation')"
                      class="text-xs"></Column>
                    <Column field="position" :header="t('catalog.option_types.labels.position')"
    ...
    <ModalDialog v-model="showValueDialog" :header="isEditingValue ? 'Edit Value' : 'Add Value'"
      ...
      <form @submit="onValueSubmit" class="flex flex-col gap-4 mt-2">
        <FormField :label="t('catalog.option_types.labels.value_name')" name="name" :error="valueErrors.name">
          <InputText v-model="vName" class="w-full rounded-xl h-11" :invalid="!!valueErrors.name"
            :placeholder="t('catalog.option_types.placeholders.value_name')" />
        </FormField>

        <FormField :label="t('catalog.option_types.labels.value_presentation')" name="presentation"
          :error="valueErrors.presentation">
          <InputText v-model="vPresentation" class="w-full rounded-xl h-11" :invalid="!!valueErrors.presentation"
            :placeholder="t('catalog.option_types.placeholders.value_presentation')" />
        </FormField>

        <FormField :label="t('catalog.option_types.labels.position')" name="position">
          <InputNumber v-model="vPosition" class="w-full rounded-xl overflow-hidden" inputClass="h-11" showButtons
            :min="0" />
        </FormField>

        <div class="flex justify-end gap-2 mt-4">
          <Button type="button" label="Cancel" severity="secondary" text @click="showValueDialog = false" />
          <Button type="submit" label="Save Value" icon="pi pi-check" :loading="submittingValue"
