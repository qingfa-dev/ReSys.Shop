...
import type { ProductImage, productRepository } from '../../products';
...
const loadImages = async () => {
    loading.value = true;
    try {
        const result = await productRepository.getImages(props.productId);
...
const handleUpload = async (payload: { file: File, role: number, alt: string, onSuccess: () => void }) => {
    try {
        const result = await productRepository.uploadImage(props.productId, payload.file, payload.role, payload.alt);
...
const onDelete = async (id: string) => {
    const result = await productRepository.deleteImage(id);
    if (result.isSuccess) {
...
const onUpdateImage = async (payload: { id: string, role: number, alt: string }) => {
    try {
        const result = await productRepository.updateImage(payload.id, { alt: payload.alt, role: payload.role });
