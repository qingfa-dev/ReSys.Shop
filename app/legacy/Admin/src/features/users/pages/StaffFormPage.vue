...
import { createStaffSchema } from '../types/staff.field'
import PageShell from '@/shared/components/navigation/PageShell.vue'
import PageHeader from '@/shared/components/navigation/PageHeader.vue'
import FormField from '@/shared/components/form/FormField.vue'
import { userRepository } from '../api/user.api'
import { roleRepository } from '../roles/api/role.api'
import type { CreateAdminUserRequest, UpdateAdminUserRequest } from '../types/user.request'
...
async function fetchRoles() {
  const res = await roleRepository.list({ pageSize: 100 })
...
async function loadUser() {
  const res = await userRepository.getById(userId.value)
...
const onSubmit = handleSubmit(async (values) => {
  ...
  try {
    if (isEditMode.value) {
      ...
      const res = await userRepository.update(userId.value, updateData)
      if (res.isSuccess) {
      ...
      const res = await userRepository.create(createData)
      if (res.isSuccess) {
