...
async function loadPermissions() {
    loading.value = true;
    try {
        const res = await permissionRepository.list({ pageSize: 1000 });
...
async function onSave() {
    ...
    try {
        ...
        for (const name of toAdd) {
            await userRepository.assignPermission(props.userId, { permissionName: name });
        }
        for (const name of toRemove) {
            await userRepository.unassignPermission(props.userId, { permissionName: name });
        }
