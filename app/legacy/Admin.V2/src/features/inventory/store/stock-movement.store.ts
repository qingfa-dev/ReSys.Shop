...
export const useStockMovementStore = defineStore('inventory-stock-movement', () => {
  const items = ref<StockMovementResponse[]>([])
  ...
  async function fetchMany() {
    ...
    error.value = null
    try {
      const result = await StockMovementApi.getMany(query.value)
      if (result.isSuccess) {
