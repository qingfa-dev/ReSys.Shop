...
describe('useStockTransferStore', () => {
  ...
  it('fetchMany success', async () => {
    mockGetMany.mockResolvedValue(pagedResult({ items: [{ id: '1', status: 'Draft' }], totalCount: 1 }))
    const store = useStockTransferStore()
  ...
  it('setFilter sets filter group and resets page', async () => {
    mockGetMany.mockResolvedValue(pagedResult())
    const store = useStockTransferStore()
    await store.setFilter({ logic: 'And', conditions: [{ field: 'status', operator: 'Equal', value: 'Draft' }], groups: [] })
    expect(mockGetMany).toHaveBeenCalled()
    expect(store.query.filters).toEqual({ logic: 'And', conditions: [{ field: 'status', operator: 'Equal', value: 'Draft' }], groups: [] })
    expect(store.query.page).toBe(1)
  })
