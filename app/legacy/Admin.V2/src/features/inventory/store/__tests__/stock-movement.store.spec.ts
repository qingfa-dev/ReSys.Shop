...
import { useStockMovementStore } from '../stock-movement.store'
...
describe('useStockMovementStore', () => {
  ...
  it('fetchMany success', async () => {
    mockGetMany.mockResolvedValue(pagedResult({ items: [{ id: '1', type: 'addition' }], totalCount: 1 }))
    const store = useStockMovementStore()
  ...
  it('setSort updates sort clause', async () => {
    mockGetMany.mockResolvedValue(pagedResult())
    const store = useStockMovementStore()
    await store.setSort('createdAt', 'Descending')
  ...
  it('setFilter sets filter group and resets page', async () => {
    mockGetMany.mockResolvedValue(pagedResult())
    const store = useStockMovementStore()
    await store.setFilter({ logic: 'And', conditions: [{ field: 'type', operator: 'Equal', value: 'addition' }], groups: [] })
    expect(mockGetMany).toHaveBeenCalled()
    expect(store.query.filters).toEqual({ logic: 'And', conditions: [{ field: 'type', operator: 'Equal', value: 'addition' }], groups: [] })
    expect(store.query.page).toBe(1)
  })
