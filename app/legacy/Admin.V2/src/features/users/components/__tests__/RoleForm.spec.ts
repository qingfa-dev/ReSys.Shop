...
beforeEach(() => {
  vi.clearAllMocks()
  mockRoute.params = {}
  mockRoute.name = 'users.roles.create'
  mockRoute.meta = {}
...
describe('RoleForm', () => {
  ...
  describe('view mode', () => {
    beforeEach(() => {
      ...
      mockRoute.name = 'users.roles.view'
      mockApiGet.mockResolvedValue(successResult({ ...mockRole }))
    })
  ...
  describe('edit mode', () => {
    beforeEach(() => {
      mockRoute.params = { id: 'r1' }
      mockRoute.name = 'users.roles.edit'
      mockApiGet.mockResolvedValue(successResult({ ...mockRole }))
    })
  ...
  describe('loading state', () => {
    it('shows loading skeleton in view mode', async () => {
      ...
      resolveGet!(successResult({ ...mockRole }))
      await flushPromises()
      await wrapper.vm.$nextTick()
  ...
  describe('error state', () => {
    it('shows error state when api.get fails', async () => {
      mockRoute.params = { id: 'r1' }
      mockRoute.name = 'users.roles.view'
      mockApiGet.mockResolvedValue(errorResult('Role not found'))
      ...
      expect(wrapper.find('.error-title').text()).toBe('Role not found')
    })
  })
})
