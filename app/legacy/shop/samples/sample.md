...
export interface ProductVariant {
  ...
  stock: number
  inStock: boolean
  price: number               // May differ per variant (e.g. plus-size markup)
}

export interface ColorOption {
  id: string
  name: string               // e.g. "Ivory", "Caramel"
  hex: string
  images: ProductImage[]
...
export interface Product {
  ...
  rating: number
  reviewCount: number
  images: ProductImage[]      // Default color images
  variants: ProductVariant[]  // Default color variants
...
> {
  images: [ProductImage, ProductImage?]   // max 2 for card hover
...
```ts
import type { Product, ProductVariant, ColorOption, Currency } from './product'
...
export interface CartItem {
  ...
  image: string
  price: number               // price at time of add (snapshot)
  currency: Currency
  addedAt: string
}
...
export interface User {
  ...
  preferences: {
    ...
    sizes: Record<string, string>  // { tops: 'M', pants: '30x32' }
  }
...
export interface Order {
  id: string
  number: string             // Human-readable: #ORD-2024-0001
  status: OrderStatus
  items: OrderItem[]
...
import type { CartItem, CartSummary, PromoCode } from '@/types/cart'
import type { Product, ProductVariant, ColorOption } from '@/types/product'
...
export const useCartStore = defineStore('cart', () => {
  ...
  const summary = computed<CartSummary>(() => {
    ...
    const tax = (subtotal - discount) * 0.09 // 9% GST
...
import { defineStore } from 'pinia'
import { ref, computed } from 'vue'
import type { FilterState, ActiveFilter, SortOption } from '@/types/filters'
...
export const useFiltersStore = defineStore('filters', () => {
  ...
  const activeFilters = computed<ActiveFilter[]>(() => {
    ...
    state.value.categories
      .filter(f => f.active)
      .forEach(f => active.push({ key: 'categories', value: f.id, label: f.label }))

    state.value.brands
      .filter(f => f.active)
      .forEach(f => active.push({ key: 'brands', value: f.id, label: f.label }))

    state.value.sizes
      .filter(f => f.active)
      .forEach(f => active.push({ key: 'sizes', value: f.id, label: f.label }))

    state.value.colors
      .filter(f => f.active)
      .forEach(f => active.push({ key: 'colors', value: f.id, label: f.label }))
    ...
    if (min !== rMin || max !== rMax) {
      active.push({ key: 'activePriceRange', value: `${min}-${max}`, label: `SGD ${min} – ${max}` })
    }
  ...
  function toggleFilter(key: 'categories' | 'brands' | 'sizes' | 'colors' | 'conditions', id: string) {
    const group = state.value[key] as { id: string; active: boolean }[]
  ...
  function removeFilter(filter: ActiveFilter) {
    ...
    const group = state.value[filter.key] as { id: string; active: boolean }[]
    const item = group.find(f => f.id === filter.value)
    if (item) item.active = false
  }

  function clearAll() {
    ;(['categories', 'brands', 'sizes', 'colors', 'conditions', 'genders', 'tags'] as const)
      .forEach(key => {
        ;(state.value[key] as { active: boolean }[]).forEach(f => (f.active = false))
      })
    state.value.activePriceRange = [state.value.priceRange.min, state.value.priceRange.max]
  }
  ...
  return {
    state, activeFilters, hasActiveFilters, activeCount,
    toggleFilter, setPriceRange, setSortBy,
    removeFilter, clearAll, setAvailableFilters,
  }
})
```
...
export const useUIStore = defineStore('ui', () => {
  ...
  const recentlyViewed = ref<string[]>([]) // product IDs, max 10
...
export const useUserStore = defineStore('user', () => {
  ...
  const defaultAddress = computed(() =>
    user.value?.addresses.find(a => a.id === user.value?.defaultAddressId)
  )
  ...
  return {
    user, orders, token, loading,
    isLoggedIn, fullName, defaultAddress,
...
export const routes: RouteRecordRaw[] = [
  // ─── Public Layout ────────────────────────────────────────────────────────
  {
    ...
    children: [
      ...
      },
      {
        path: 'new-in',
        name: 'new-in',
        component: () => import('@/pages/new-in.vue'),
        meta: { title: 'New In — Maison' },
      },
      {
        path: 'sale',
        name: 'sale',
        component: () => import('@/pages/sale.vue'),
        meta: { title: 'Sale — Maison' },
      },
      {
        path: 'designers',
        name: 'designers',
        component: () => import('@/pages/designers/index.vue'),
        meta: { title: 'Designers — Maison' },
      },
      {
        path: 'designers/:slug',
        name: 'designer',
        component: () => import('@/pages/designers/[slug].vue'),
        meta: { title: 'Designer — Maison' },
      },
      {
        path: ':gender(women|men|kids)',
        name: 'gender',
        component: () => import('@/pages/[gender]/index.vue'),
      },
      {
        path: ':gender(women|men|kids)/:category',
        name: 'category',
        component: () => import('@/pages/[gender]/[category].vue'),
        meta: { requiresFilters: true },
      },
      {
        path: 'product/:slug',
        name: 'product',
      ...
      },
      {
        ...
        meta: { title: 'Search — Maison' },
      },
      {
        path: 'lookbook',
        name: 'lookbook',
        component: () => import('@/pages/lookbook.vue'),
        meta: { title: 'Lookbook — Maison' },
      },
      {
        path: 'wishlist',
        name: 'wishlist',
  ...
  // ─── Checkout Layout ──────────────────────────────────────────────────────
  {
    ...
    children: [
      ...
      },
      {
        ...
        component: () => import('@/pages/checkout/review.vue'),
        meta: { title: 'Review Order — Maison' },
      },
      {
        path: 'confirmation/:orderId',
        name: 'order-confirmation',
        component: () => import('@/pages/checkout/confirmation.vue'),
        meta: { title: 'Order Confirmed — Maison' },
      },
    ],
  },
  // ─── Auth Layout ──────────────────────────────────────────────────────────
  {
    ...
    children: [
      ...
      },
      {
        ...
        meta: { title: 'Reset Password — Maison', guestOnly: true },
      },
      {
        path: 'reset-password',
        name: 'reset-password',
    ...
    children: [
      {
        ...
        meta: { title: 'My Account — Maison' },
      },
      {
        path: 'orders',
        name: 'account-orders',
        component: () => import('@/pages/account/orders.vue'),
        meta: { title: 'My Orders — Maison' },
      },
      {
        path: 'orders/:id',
        name: 'account-order-detail',
        component: () => import('@/pages/account/orders/[id].vue'),
      },
      {
        path: 'profile',
        name: 'account-profile',
        component: () => import('@/pages/account/profile.vue'),
        meta: { title: 'Profile — Maison' },
      },
      {
        path: 'addresses',
        name: 'account-addresses',
        component: () => import('@/pages/account/addresses.vue'),
        meta: { title: 'Addresses — Maison' },
      },
      {
        path: 'returns',
        name: 'account-returns',
        component: () => import('@/pages/account/returns.vue'),
        meta: { title: 'Returns — Maison' },
      },
      {
        path: 'payment-methods',
        name: 'account-payment',
        component: () => import('@/pages/account/payment-methods.vue'),
        meta: { title: 'Payment Methods — Maison' },
      },
      {
        path: 'preferences',
        name: 'account-preferences',
        component: () => import('@/pages/account/preferences.vue'),
        meta: { title: 'Preferences — Maison' },
      },
    ],
  },
  // ─── Error Pages ──────────────────────────────────────────────────────────
  {
    ...
    meta: { title: 'Page Not Found — Maison' },
  },
  {
    path: '/:pathMatch(.*)*',
    redirect: '/404',
...
const router = createRouter({
  ...
  scrollBehavior(to, from, savedPosition) {
    ...
    if (to.name === from.name) return false   // Filter changes — no scroll
...
router.beforeEach(async (to, _from, next) => {
  ...
  // Guest-only guard (logged-in users redirected from auth pages)
...
import type { Product, ProductListItem, Review, SizeGuide } from '@/types/product'
import type { ProductsRequest } from '@/types/api'

export const productsService = {
  async list(params: ProductsRequest): Promise<ApiResponse<ProductListItem[]>> {
  ...
  async getRelated(productId: string, limit = 8): Promise<ProductListItem[]> {
    const { data } = await api.get(`/products/${productId}/related`, { params: { limit } })
    return data.data
  },

  async getNewIn(limit = 16): Promise<ProductListItem[]> {
    const { data } = await api.get('/products', { params: { isNew: true, limit } })
    return data.data
  },

  async getSale(limit = 16): Promise<ProductListItem[]> {
    const { data } = await api.get('/products', { params: { isSale: true, limit } })
    return data.data
  },

  async getReviews(productId: string, page = 1): Promise<ApiResponse<Review[]>> {
    const { data } = await api.get(`/products/${productId}/reviews`, { params: { page } })
    return data
  },

  async getSizeGuide(sizeGuideId: string): Promise<SizeGuide> {
    const { data } = await api.get(`/size-guides/${sizeGuideId}`)
    return data.data
  },

  async search(query: string, params?: Partial<ProductsRequest>): Promise<ApiResponse<ProductListItem[]>> {
    const { data } = await api.get('/products/search', { params: { q: query, ...params } })
    return data
  },

  async getByBrand(brandSlug: string, params?: Partial<ProductsRequest>): Promise<ApiResponse<ProductListItem[]>> {
    const { data } = await api.get(`/brands/${brandSlug}/products`, { params })
    return data
  },
}
```
...
export const ordersService = {
  ...
  async requestReturn(orderId: string, items: { itemId: string; reason: string }[]): Promise<void> {
    await api.post(`/orders/${orderId}/returns`, { items })
  },
}
```
...
export function useProductList(params: ProductsRequest) {
  const queryKey = computed(() => ['products', params])
  ...
  return { products, meta, isLoading, isError, error, refetch }
}

export function useProduct(slug: string) {
  const { data: product, isLoading, isError } = useQuery({
    queryKey: ['product', slug],
    queryFn: () => productsService.get(slug),
...
export function useRelatedProducts(productId: string, limit = 8) {
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => productsService.getRelated(productId, limit),
    enabled: !!productId,
  })
  return { related: computed(() => data.value ?? []), isLoading }
}
```
...
export function useCart() {
  ...
  function addToCart(product: Product, variant: ProductVariant, color: ColorOption, qty = 1) {
    ...
    cartStore.addItem(product, variant, color, qty)
  ...
  function toggleWishlist(product: ProductListItem) {
    if (!user.isLoggedIn) {
      toast.add({
        ...
        detail: 'Please sign in to save items to your wishlist',
        life: 3000,
      })
    ...
    const action = store.isWishlisted(product.id) ? 'Added to' : 'Removed from'
    toast.add({
      severity: 'success',
...
export function useScrollLock() {
  ...
  return {
    ...
    unlock: () => { locked.value = false },
    toggle: () => { locked.value = !locked.value },
  }
}
```
...
export function useBreakpoint() {
  ...
  return { width, isSm, isMd, isLg, isXl, is2xl, isMobile, isTablet, isDesktop }
}
```
...
export function useCurrency() {
  ...
  const currency = computed(() => user.user?.preferences.currency ?? 'SGD')

  function format(amount: number, cur?: string): string {
