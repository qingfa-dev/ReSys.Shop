...
<template>
  <div class="bg-white rounded-xl border border-gray-200 p-6">
    <h2 class="text-lg font-semibold text-gray-900 mb-4">Confirm Order</h2>
    <div class="border border-gray-200 rounded-lg divide-y divide-gray-200 mb-6">
      <div class="flex justify-between px-4 py-3 text-sm">
        <span class="text-gray-600">Items</span>
        <span class="text-gray-900 font-medium">{{ cart.itemCount }}</span>
      </div>
      <div class="flex justify-between px-4 py-3 text-sm">
        <span class="text-gray-600">Subtotal</span>
        <span class="text-gray-900 font-medium">{{ formatVnd(cart.subtotal) }}</span>
      </div>
      <div class="flex justify-between px-4 py-3 text-sm">
        <span class="text-gray-600">Shipping</span>
        <span class="text-gray-400">Calculated at delivery</span>
      </div>
      <div class="flex justify-between px-4 py-3 text-sm">
        <span class="text-gray-600">Contact email</span>
        <span class="text-gray-900 font-medium">{{ checkout.email }}</span>
      </div>
    </div>
