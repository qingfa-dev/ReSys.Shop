...
describe('CheckoutView', () => {
  ...
  it('force-refreshes the cart and validates checkout when advancing to review', async () => {
    mockedCartApi.getCart.mockResolvedValue(
      ok({ id: 'cart-1', itemTotal: 90, total: 90, currency: 'USD', itemCount: 2, checkoutState: 'PickPaymentMethod', shippingMethodId: null, shipAddressId: null, email: null, shipmentTotal: 0, adjustmentTotal: 0, shippingAdjustment: null, shippingCalculation: null, adjustments: [], items: [lineItem] }),
    )
  ...
  it('renders the payment method radio list on the payment panel', async () => {
    mockedCartApi.getCart.mockResolvedValue(
      ok({ id: 'cart-1', itemTotal: 90, total: 90, currency: 'USD', itemCount: 2, checkoutState: 'PickDeliveryMethod', shippingMethodId: null, shipAddressId: null, email: null, shipmentTotal: 0, adjustmentTotal: 0, shippingAdjustment: null, shippingCalculation: null, adjustments: [], items: [lineItem] }),
    )
    const { wrapper } = await mountView(true)
