...
const imageAlt = computed(() => props.product.masterVariant?.images?.[0]?.alt ?? props.product.name)
...
const compareAtPrice = computed(() => props.product.masterVariant?.prices.find(p => p.compareAtAmount != null)?.compareAtAmount ?? null)
...
const wishlistLabel = computed(() => (isWishlisted.value ? 'Remove from wishlist' : 'Add to wishlist'))
...
const menuItems = computed<MenuItem[]>(() => [
  {
    label: 'Quick view',
    icon: 'pi pi-eye',
    command: ({ originalEvent }) => quickViewPopover.value?.show(originalEvent, quickViewButton.value?.$el),
  },
  { label: 'Add to cart', icon: 'pi pi-shopping-cart', command: ({ originalEvent }) => onAddToCart(originalEvent) },
  { label: 'Wishlist', icon: isWishlisted.value ? 'pi pi-heart-fill' : 'pi pi-heart', command: () => void toggleWishlist() },
])
</script>
