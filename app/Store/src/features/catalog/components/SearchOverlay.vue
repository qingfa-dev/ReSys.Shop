...
function onKeyDown(e: KeyboardEvent): void {
  if (!search.isOpen.value) return
  if (e.key === 'Escape') { e.preventDefault(); search.close(); return }
  if (!search.results.value.length) return
  if (e.key === 'ArrowDown') {
    e.preventDefault()
    search.selectedIndex.value = (search.selectedIndex.value + 1) % search.results.value.length
  } else if (e.key === 'ArrowUp') {
    e.preventDefault()
    search.selectedIndex.value = (search.selectedIndex.value - 1 + search.results.value.length) % search.results.value.length
  } else if (e.key === 'Enter') {
    e.preventDefault()
...
<template>
  ...
  <Teleport to="body">
    <div v-if="search.isOpen" class="fixed inset-0 z-50 flex items-start justify-center pt-[10vh]">
      <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" @click="search.close()" />
      <!-- Section: Content Card — search input and results -->
      <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-2xl mx-4 overflow-hidden max-h-[70vh] flex flex-col">
        ...
        <div v-if="search.loading" class="p-8 text-center text-neutral-400">
          <i class="pi pi-spin pi-spinner text-2xl mb-2 block" />
          <p class="text-sm">Searching...</p>
        </div>

        <!-- Section: Results List — navigable product results -->
        <div v-else-if="search.results.length" class="overflow-y-auto max-h-[50vh]">
          <ul>
            <li
              ...
              :class="index === search.selectedIndex ? 'bg-neutral-100' : 'hover:bg-neutral-50'"
              @mouseenter="search.selectedIndex = index"
              @click="search.navigateToResult(index)"
            >
              <img
              ...
              <i v-if="index === search.selectedIndex" class="pi pi-arrow-right text-neutral-400 ml-auto shrink-0" />
            </li>
          </ul>
        </div>

        <!-- Section: Empty State — shown when query entered but no results -->
        <div v-else-if="search.query && !search.loading" class="p-8 text-center text-neutral-400">
          <i class="pi pi-search text-2xl mb-2 block" />
          <p class="text-sm">No results found for "{{ search.query }}"</p>
        </div>
        ...
        <div v-if="search.error" class="px-4 py-2 bg-red-50 text-red-600 text-sm text-center">
          {{ search.error }}
        </div>

        <!-- Section: Keyboard Hints — navigation shortcuts -->
        <div class="px-4 py-2 border-t border-neutral-100 flex items-center gap-4 text-[11px] text-neutral-400">
