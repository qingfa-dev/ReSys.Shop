  ...
  <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <Breadcrumb :model="breadcrumbItems" :home="homeItem" class="mb-6">
      <template #item="{ item, label, icon, props }">
        ...
        <RouterLink
          ...
          v-bind="props.action"
          class="flex items-center gap-1 text-surface-500 transition-colors hover:text-primary-600 dark:text-surface-400"
        >
    ...
    <div v-else-if="detail.product" class="grid grid-cols-1 gap-10 lg:grid-cols-2">
      ...
      <div class="min-w-0">
        ...
        <div
          v-else
          class="flex aspect-square items-center justify-center rounded-2xl bg-surface-50 dark:bg-surface-800"
        >
      ...
      <div class="space-y-6">
        <div>
          ...
          <div class="mt-2 flex items-center gap-3">
            ...
            <span v-if="ratingCount !== null && ratingCount > 0" class="text-sm text-surface-500 dark:text-surface-400">
              {{ ratingCount }} reviews
            </span>
            ...
            <TabPanel value="details">
              <dl class="grid grid-cols-1 gap-2 sm:grid-cols-2">
                <div
                  ...
                  :key="row.label"
                  class="flex justify-between gap-4 border-b border-surface-100 py-2 dark:border-surface-800"
                >
                  <dt class="text-sm text-surface-500 dark:text-surface-400">{{ row.label }}</dt>
    ...
    <section v-if="detail.product && detail.relatedProducts.length > 0" class="mt-16">
      <h2 class="mb-6 text-2xl font-semibold tracking-tight text-surface-900 dark:text-surface-100">
        You may also like
      </h2>
