...
async function fetchProfile(): Promise<void> {
  ...
  try {
    ...
    error.value = e instanceof HttpError ? e.errors[0]?.message ?? 'Failed to load profile' : 'Failed to load profile'
  }
  loading.value = false
}
...
async function updateProfile(req: UpdateProfileRequest): Promise<boolean> {
  ...
  try {
    ...
    error.value = e instanceof HttpError ? e.errors[0]?.message ?? 'Could not update profile' : 'Could not update profile'
    profile.value = prev
    saving.value = false
    return false
  }
...
async function deleteProfile(): Promise<boolean> {
  ...
  try {
    ...
    error.value = e instanceof HttpError ? e.errors[0]?.message ?? 'Could not delete profile' : 'Could not delete profile'
    saving.value = false
    return false
  }
}
