...
export class CartService implements ICartService {
  ...
  async applyCoupon(code: string): Promise<Result<Cart>> {
    const response = await this.cartRepo.applyCoupon(code)
    return resultMap(response, mapCartResponseToEntity)
  }

  async removeCoupon(): Promise<Result<Cart>> {
    const response = await this.cartRepo.removeCoupon()
    return resultMap(response, mapCartResponseToEntity)
  }
}
