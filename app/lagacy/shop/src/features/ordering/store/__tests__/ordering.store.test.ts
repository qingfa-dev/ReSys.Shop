...
describe('useCartStore', () => {
  ...
  describe('applyCoupon', () => {
    it('should apply coupon successfully', async () => {
      const discountedCart = { ...mockCart, discount: 10 }
      const mockResult = { isSuccess: true, isFailure: false, statusCode: 200, data: discountedCart }
      vi.mocked(cartService.applyCoupon).mockResolvedValue(mockResult)

      const store = useCartStore()
      await store.applyCoupon('SAVE10')

      expect(store.cart?.discount).toBe(10)
    })

    it('should throw on invalid coupon', async () => {
      const mockResult = { isSuccess: false, isFailure: true, statusCode: 400, message: 'Invalid coupon', errors: [] }
      vi.mocked(cartService.applyCoupon).mockResolvedValue(mockResult)

      const store = useCartStore()
      const fn = () => store.applyCoupon('INVALID')
  ...
  describe('removeCoupon', () => {
    it('should remove coupon', async () => {
      const mockResult = { isSuccess: true, isFailure: false, statusCode: 200, data: mockCart }
      vi.mocked(cartService.removeCoupon).mockResolvedValue(mockResult)
