...
export class MockCartRepository {
  ...
  async applyCoupon(code: string): Promise<CartSingleResponse> {
    if (code === 'SAVE10') {
      mockCart = { ...mockCart, discount: mockCart.subtotal * 0.1 }
      recalculateCart()
    }
    return { isSuccess: true, isFailure: false, statusCode: 200, data: mapToCartResponse(mockCart) }
  }

  async removeCoupon(): Promise<CartSingleResponse> {
    mockCart = { ...mockCart, discount: 0 }
    recalculateCart()
    return { isSuccess: true, isFailure: false, statusCode: 200, data: mapToCartResponse(mockCart) }
  }
