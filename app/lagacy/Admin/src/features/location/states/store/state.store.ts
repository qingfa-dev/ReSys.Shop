...
import type { CreateStateRequest, UpdateStateRequest } from '../types/state.request'
...
export const useStateStore = defineStore('state', () => {
  ...
  async function fetchStates(params?: Record<string, unknown>) {
    loading.value = true
    const result = await stateRepository.list(params)
    if (result.isSuccess) {
  ...
  async function fetchStateById(id: string) {
    loading.value = true
    const result = await stateRepository.getById(id)
    loading.value = false
    return result
  }
