...
// Initialize entries from modelValue
const initialize = () => {
...
const sync = () => {
    ...
    entries.value.forEach(entry => {
        if (entry.key.trim()) {
            // Attempt to parse JSON if it looks like an object/array, otherwise use as is
            let val = entry.value;
...
// Watch for external changes (optional, but good for form resets)
watch(() => modelValue.value, (newVal, oldVal) => {
    // Only re-initialize if the keys changed externally (e.g. form reset or load)
