...
function updateColors(type: string, color: { name: string; palette?: Record<string, string> }): void {
    if (type === 'primary') {
...
function applyTheme(type: string, color: { name: string; palette?: Record<string, string> }): void {
    if (type === 'primary') {
...
function onPresetChange(): void {
    ...
    const surfacePalette = surfaces.value.find((s: { name: string }) => s.name === layoutConfig.surface)?.palette;

    $t().preset(presetValue as never).preset(getPresetExt() as never).surfacePalette(surfacePalette!).use({ useDefaultOptions: true });
}
</script>
