...
import type { ParsedApiError } from './error.type'

export function normalizeServerErrors(errors: unknown): Record<string, string[]> {
  if (!errors) return {}
  ...
  if (axiosError.isAxiosError || axiosError.response || axiosError.request) {
    ...
    if (apiData && typeof apiData === 'object') {
      ...
      return {
        ...
        errors: normalizeServerErrors(rawErrors),
        errorCode,
      }
    }
