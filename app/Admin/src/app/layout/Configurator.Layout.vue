...
const primaryColors = ref([
  { name: 'noir', palette: {} as Record<string, string> },
...
function updateColors(type: 'primary' | 'surface', color: { name: string; palette: Record<string, string> }) {
  if (type === 'primary') {
...
function applyTheme(type: 'primary' | 'surface', color: { name: string; palette: Record<string, string> }) {
  if (type === 'primary') {
...
function onPresetChange() {
  ...
  const surface = surfaces.value.find((s) => s.name === layoutConfig.surface)?.palette
  $t().preset(presetValue).preset(getPresetExt()).surfacePalette(surface).use({ useDefaultOptions: true })
}
</script>

<template>
  <div
    class="config-panel hidden absolute top-[3.25rem] right-0 w-64 p-4 bg-surface-0 dark:bg-surface-900 border border-surface rounded-lg origin-top shadow-[0px_3px_5px_rgba(0,0,0,0.02),0px_0px_2px_rgba(0,0,0,0.05),0px_1px_4px_rgba(0,0,0,0.08)]"
  >
