...
const mockCountryGetMany = vi.hoisted(() => vi.fn())
...
beforeEach(() => {
  vi.clearAllMocks()
  mockRoute.params = {}
  mockRoute.name = 'location.states.create'
  mockRoute.meta = {}
  ...
  mockCountryGetMany.mockResolvedValue({ isSuccess: true, items: [{ id: 'c1', name: 'United States' }, { id: 'c2', name: 'Canada' }] })
})
...
describe('StateForm', () => {
  describe('create mode', () => {
    it('renders form fields and save/cancel actions', () => {
      ...
      expect(wrapper.find('.p-select-stub').exists()).toBe(true)
    })
  })

  describe('view mode', () => {
    beforeEach(() => {
      mockRoute.params = { id: 's1' }
      mockRoute.name = 'location.states.view'
      mockApiGet.mockResolvedValue(successResult({ ...mockState }))
    })
    ...
    it('shows edit button in header actions', async () => {
      ...
      expect(editBtn.text()).toContain('location.states.actions.edit')
    })
  })
  ...
  describe('edit mode', () => {
    beforeEach(() => {
      mockRoute.params = { id: 's1' }
      mockRoute.name = 'location.states.edit'
      mockApiGet.mockResolvedValue(successResult({ ...mockState }))
    })
  ...
  describe('loading state', () => {
    it('shows loading skeleton while fetching in view mode', async () => {
      ...
      mockCountryGetMany.mockReturnValue(Promise.resolve({ isSuccess: true, items: [{ id: 'c1', name: 'United States' }] }))
      ...
      resolveGet!(successResult({ ...mockState }))
      await flushPromises()
      await wrapper.vm.$nextTick()
  ...
  describe('error state', () => {
    it('shows error state when api.get fails', async () => {
      mockRoute.params = { id: 's1' }
      mockRoute.name = 'location.states.view'
      mockApiGet.mockResolvedValue(errorResult('State not found'))
      ...
      expect(wrapper.find('.error-title').text()).toBe('State not found')
