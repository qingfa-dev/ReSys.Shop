...
beforeEach(() => {
  vi.clearAllMocks()
  mockRoute.params = {}
  mockRoute.name = 'location.countries.create'
  mockRoute.meta = {}
...
describe('CountryForm', () => {
  ...
  describe('view mode', () => {
    beforeEach(() => {
      mockRoute.params = { id: 'c1' }
      mockRoute.name = 'location.countries.view'
      mockApiGet.mockResolvedValue(successResult({ ...mockCountry }))
    })
    ...
    it('shows edit button in header actions', async () => {
      ...
      expect(editBtn.text()).toContain('location.countries.actions.edit')
    })
  })
  ...
  describe('edit mode', () => {
    beforeEach(() => {
      mockRoute.params = { id: 'c1' }
      mockRoute.name = 'location.countries.edit'
      mockApiGet.mockResolvedValue(successResult({ ...mockCountry }))
    })
  ...
  describe('loading state', () => {
    it('shows loading skeleton while fetching in view mode', async () => {
      ...
      resolveGet!(successResult({ ...mockCountry }))
      await flushPromises()
      await wrapper.vm.$nextTick()
