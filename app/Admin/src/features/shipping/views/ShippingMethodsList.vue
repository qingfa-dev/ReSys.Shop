...
const selectedItems = ref<ShippingMethodListItem[]>([])
const search = ref('')
...
function bulkConfirmDelete() {
  ...
  confirm.require({
    message: `Are you sure you want to delete ${selectedItems.value.length > 1 ? 'these shipping methods' : 'this shipping method'}?`,
    header: 'Confirm Delete',
    icon: 'pi pi-exclamation-triangle',
    rejectLabel: 'Cancel',
    acceptLabel: 'Delete',
    ...
    accept: async () => {
      ...
      let failed = 0
      for (const id of ids) {
        const result = await ShippingMethodApi.deleteShippingMethod(id)
      ...
      if (failed === 0) {
        notify.success(
          ids.length > 1 ? 'Shipping methods deleted' : 'Shipping method deleted',
          ids.length > 1
            ? `${ids.length} shipping methods have been removed.`
