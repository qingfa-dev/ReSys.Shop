...
import type { StockLocationResponse } from '../types'
import { StockLocationApi } from '../api'
import type { FilterGroup, SortDirection } from '@/shared/models/querying'

export const useStockLocationStore = defineStore('inventory-stock-location', () => {
  const items = ref<StockLocationResponse[]>([])
  ...
  async function fetchMany() {
    ...
    error.value = null
    try {
      const result = await StockLocationApi.getMany(query.value)
      if (result.isSuccess) {
