...
export class RolePermissionApi {
  ...
  static async assign(roleId: string, data: UserRoleIdsRequest): Promise<Result<void>> {
    const res = await apiClient.put<Result<void>>(`/identity/roles/${roleId}/permissions/assign`, data)
    return res.data
  }
  static async revoke(roleId: string, data: UserRoleIdsRequest): Promise<Result<void>> {
    const res = await apiClient.delete<Result<void>>(`/identity/roles/${roleId}/permissions/revoke`, { data })
    return res.data
  }
  static async sync(roleId: string, data: UserRoleIdsRequest): Promise<Result<void>> {
