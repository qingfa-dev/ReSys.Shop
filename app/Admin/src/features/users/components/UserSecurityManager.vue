...
async function onResetPassword() {
    confirm.require({
        ...
        accept: async () => {
            ...
            const res = await userRepository.resetPassword(props.user.id, { new_password: newPassword });
            if (res.isSuccess) {
                confirm.require({
...
async function onUnlock() {
    loading.value = true;
    try {
        const res = await userRepository.unlockAccount(props.user.id);
        if (res.isSuccess) {
...
async function onVerify() {
    loading.value = true;
    try {
        const res = await userRepository.verifyAccount(props.user.id, { verifyEmail: true, verifyPhone: true });
        if (res.isSuccess) {
