...
<template>
    <Dialog header="Refund Payment" visible modal class="w-full max-w-md" @update:visible="$emit('close')">
        ...
        <template #footer>
            <div class="flex justify-end gap-3 pt-4 border-t border-surface-100 dark:border-surface-800">
                ...
                <Button label="Process Refund" icon="pi pi-undo" severity="danger" @click="onSave" class="rounded-xl" />
            </div>
        </template>
    </Dialog>
</template>
