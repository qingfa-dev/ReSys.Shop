...
- Same **normalised** `baseColour`

Colour normalisation merges 46 raw labels (``"Navy Blue"``, ``"Turquoise Blue"``) into 11 perceptual groups (``"Blue"``). This makes the ground truth reflect human visual perception rather than exact marketing-label matching.  Without normalisation, 21% of items had no colour-mate — the benchmark would be too strict.
...
**Colour normalisation example:** ``"Navy Blue"`` → ``"Blue"``, ``"Turquoise Blue"`` → ``"Blue"``. A Navy Blue T-shirt is relevant to a Turquoise Blue T-shirt — they look visually similar despite different marketing labels.
...
The benchmark expects a JSON split file (auto-generated by `GroundTruth.generate_splits()`):

```json
[
  {
    "image_path": "images/1163.jpg",
    "label": "Apparel/Topwear/Black",
    "product_id": "1163"
  },
  {
...
- `label` — ground-truth relevance key (masterCategory/subCategory/normalisedColour)
- `product_id` — unique identifier (matches `styles.csv` id)
- `label_pattern` — optional secondary label with pattern (e.g. `"Apparel/Topwear/Black/Checked"`) when enriched dataset used
...
- [ ] Run validation: `uv run benchmark run --dataset-root data/raw/fashion-product-images-small --models fashion-clip` (first run validates images)
...
uv run benchmark enrich \
    --dataset-root data/raw/fashion-product-images \
